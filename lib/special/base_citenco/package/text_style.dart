...
class BaseText {
  ...
  //source
  TextStyle fontFamilyStyle() => TextStyle(
      fontFamily: fontFamily, color: BasePKG().color.text, height: 1.2);
...
//primary
  TextStyle smallPrimary() =>
      smallNormal().copyWith(color: BasePKG().color.primaryColor);

//text feild
  TextStyle valueLabel() => fontFamilyStyle()
      .copyWith(fontSize: 15, height: 1.17, fontWeight: FontWeight.w400);

  TextStyle label() => fontFamilyStyle().copyWith(
      fontSize: 16, fontWeight: FontWeight.w400, color: Color(0xff7F7F7F));

  TextStyle labelSmall() => fontFamilyStyle().copyWith(
      fontSize: 13, fontWeight: FontWeight.w400, color: Color(0xff7F7F7F));

//common
  TextStyle appBarAction() =>
      smallNormal().copyWith(color: BasePKG().color.appBarTitle);

  TextStyle point() =>
      normalBold().copyWith(color: BasePKG().color.rewardPointText);

  TextStyle description() =>
      smallNormal().copyWith(color: BasePKG().color.description);

  TextStyle descriptionGrey() =>
      smallLowerNormal().copyWith(color: Color(0xff958E84));

//dialog
  TextStyle messageDialog() =>
      smallNormal().copyWith(color: BasePKG().color.text.withOpacity(.8));

  TextStyle titleDialog() =>
      largeLowerBold().copyWith(color: BasePKG().color.titleDialog);

  TextStyle contentWidget() =>
      normalLowerNormal().copyWith(color: BasePKG().color.invertTextColor);

  //announcement
  TextStyle titleNotify() => smallUpperBold();

  TextStyle descriptionNotify() =>
      smallNormal().copyWith(color: BasePKG().color.descriptionNotify);

  TextStyle titleNotifySeen() =>
      smallUpperBold().copyWith(color: BasePKG().color.titleNotifySeen);

  TextStyle descriptionNotifySeen() =>
      smallNormal().copyWith(color: BasePKG().color.description);

  TextStyle dateNotify() =>
      tinyUpperNormal().copyWith(color: BasePKG().color.titleNotifySeen);

  //home
  TextStyle homeMenuHeader() =>
      smallNormal().copyWith(color: BasePKG().color.homeMenuHeader);

  TextStyle homeTitleList() =>
      largeUpperNormal().copyWith(color: Color(0xff1F1F1F));

  TextStyle homeReadmore() =>
      smallBold().copyWith(color: BasePKG().color.primaryColor);

  TextStyle homeDiscount() =>
      normalLowerNormal().copyWith(color: BasePKG().color.hightLight);

  TextStyle appBarTitle() =>
      normalUpperMedium().copyWith(color: BasePKG().color.appBarTitle);
}
