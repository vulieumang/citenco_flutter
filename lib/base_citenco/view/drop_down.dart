...
class DropDownView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    bool _disable = items.isEmpty || index == -1;
    return Stack(
      ...
      children: <Widget>[
        Container(
          ...
          child: Row(
            children: <Widget>[
              Expanded(
                child: Text(_disable ? '' : items[index],
                    style: BasePKG().text!.smallUpperMedium().copyWith()),
              ),
              Transform.rotate(
                child: SvgPicture.asset(
                  "lib/special/base_citenco/asset/image/arrow_more.svg",
                  height: BasePKG().convert(15),
                ),
                angle: pi / 2,
              )
            ],
        ...
        Opacity(
          ...
          child: DropdownButton<int>(
              ...
              items: List.generate(items.length, (index) {
                return DropdownMenuItemCNV<int>(
...
const double _kMenuItemCNVHeight = kMinInteractiveDimension;
const double _kDenseButtonHeight = 24.0;
const EdgeInsets _kMenuItemCNVPadding = EdgeInsets.symmetric(horizontal: 16.0);
...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final double selectedItemOffset =
        selectedIndex! * _kMenuItemCNVHeight + kMaterialListPadding.top;
    final Tween<double> top = Tween<double>(
      begin: selectedItemOffset.clamp(0.0, size.height - _kMenuItemCNVHeight),
      end: 0.0,
    );

    final Tween<double> bottom = Tween<double>(
      begin:
          (top.begin! + _kMenuItemCNVHeight).clamp(_kMenuItemCNVHeight, size.height),
      end: size.height,
    );
            ...
            child: ScrollConfiguration(
              ...
              child: Scrollbar(
                child: ListView(
                  ...
                  itemExtent: _kMenuItemCNVHeight,
                  shrinkWrap: true,
                  children: children,
                ),
              ),
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double maxHeight =
        math.max(0.0, constraints.maxHeight - 2 * _kMenuItemCNVHeight);
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  final List<DropdownMenuItemCNV<T>>? items;
...
class _DropdownRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double maxMenuHeight = availableHeight - 2.0 * _kMenuItemCNVHeight;

    final double buttonTop = buttonRect!.top;
    final double buttonBottom = math.min(buttonRect!.bottom, availableHeight);

    // If the button is placed on the bottom or top of the screen, its top or
    // bottom may be less than [_kMenuItemCNVHeight] from the edge of the screen.
    // In this case, we want to change the menu limits to align with the top
    // or bottom edge of the button.
    final double topLimit = math.min(_kMenuItemCNVHeight, buttonTop);
    final double bottomLimit =
        math.max(availableHeight - _kMenuItemCNVHeight, buttonBottom);

    final double selectedItemOffset =
        selectedIndex! * _kMenuItemCNVHeight + kMaterialListPadding.top;

    double menuTop = (buttonTop - selectedItemOffset) -
        (_kMenuItemCNVHeight - buttonRect!.height) / 2.0;
    final double preferredMenuHeight =
        (items!.length * _kMenuItemCNVHeight) + kMaterialListPadding.vertical;
...
class DropdownMenuItemCNV<T> extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropdownMenuItemCNV({
    Key? key,
    this.value,
    required this.child,
  }) : super(key: key);
...
/// Typically, an enum is used. Each [DropdownMenuItemCNV] in [items] must be
...
///           .map<DropdownMenuItemCNV<String>>((String value) {
///             return DropdownMenuItemCNV<String>(
///               value: value,
///               child: Text(value),
///             );
///           })
...
/// See also:
///
///  * [DropdownMenuItemCNV], the class used to represent the [items].
...
class DropdownButton<T> extends StatefulWidget {
  ...
  /// must be equal to one of the [DropDownMenuItemCNV] values. If [items] or
  ...
  DropdownButton({
    ...
    this.disable: false,
            ...
            value == null ||
            items
                    .where((DropdownMenuItemCNV<T> item) => item.value == value)
                    .length ==
                1),
  ...
  /// The value of the currently selected [DropdownMenuItemCNV], or null if no
  ...
  /// [BasePKG().color.card10] when it is [Brightness.dark]
  ...
  /// [ThemeData.brightness] is [Brightness.light] and to
  /// [BasePKG().color.card70] when it is [Brightness.dark]
...
class _DropdownButtonState<T> extends State<DropdownButton<T>>
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items
                ?.where(
                    (DropdownMenuItemCNV<T> item) => item.value == widget.value)
                .length ==
            1);
    _selectedIndex = null;
  ...
  Future<void> _handleTap() async {
    ...
    _dropdownRoute = _DropdownRoute<T>(
      ...
      padding: _kMenuItemCNVPadding.resolve(textDirection),
  ...
  // When isDense is true, reduce the height of this button from _kMenuItemCNVHeight to
...
class DropdownButtonFormField<T> extends FormField<T> {
  ...
  DropdownButtonFormField({
    Key? key,
    T? value,
    required List<DropdownMenuItemCNV<T>> items,
    this.onChanged,
